...
void analyse_dcl_vtype_ID(ContextSensitiveTree* tree, const string& indent){
    assert(tree->getParams()->rule == "dcl vtype ID");
    throw "analyse_dcl_vtype_ID not implemented";
}
void analyse_dcl_vtype_ID_EQUALS_expr(ContextSensitiveTree* tree, const string& indent){

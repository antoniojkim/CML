...
typedef void (*AnalysisFunction)(ContextSensitiveTree* tree, const std::string& indent);
...
\n""")
        ...
        for rules in productions[root]:
            ...
            out.write(f"void {fname}(ContextSensitiveTree* tree, const string& indent){{")
            if fname in existingImplementations:
                out.write(existingImplementations[fname])
                del existingImplementations[fname]
            else:
        ...
        for fname in existingImplementations:
            out.write(f"void {fname}(ContextSensitiveTree* tree, const string& indent){{")
            out.write(existingImplementations[fname])
            out.write("\n")

#ifndef __CML_TENSORS_TENSOR4D_H__
#define __CML_TENSORS_TENSOR4D_H__
...
namespace cml {
    ...
    inline tensor<T> make_tensor4d(Args&&... args);
    template <typename T = float>
    inline tensor<T> make_tensor(const int& R, const int& C, const bool& computeGrad = false);
    template <class T>
    inline tensor<T> make_tensor(Eigen::Tensor<T, 4>& m, const bool& computeGrad = false);
    template <class T>
    inline tensor<T> make_tensor(Eigen::Tensor<T, 4>&& m, const bool& computeGrad = false);
    ...
    class Tensor4D: public Tensor<T> {
        ...
        public:
            ...
            Tensor4D(const int& N, const int& C, const int& H, const int& W, const bool& computeGrad = false):
                Tensor<T>(computeGrad, {C, H, W}, TensorType::_4D_), t{N, C, H, W}, N{N}, C{C}, H{H}, W{W} {}

            inline DMatrix<T>& data() override {
                return Eigen::Map<DMatrix<T>>(t.data(), N, C*H*W);
            }
...
//     */
    ...
    inline tensor<T> make_tensor3d(Args&&... args) {
        return std::make_shared<Tensor4D<T>>(std::forward<Args>(args)...);
    }
    template <typename T = float>
    inline tensor<T> make_tensor(const int& C, const int& H, const int& W, const bool& computeGrad) {
        return std::make_shared<Tensor4D<T>>(C, H, W, computeGrad);
    }
    template <typename T = float>
    inline tensor<T> make_tensor(Eigen::Tensor<T, 3>& t, const bool& computeGrad) {
        return std::make_shared<Tensor4D<T>>(t, computeGrad);
    }
    template <typename T = float>
    inline tensor<T> make_tensor(Eigen::Tensor<T, 3>&& t, const bool& computeGrad) {
        return std::make_shared<Tensor4D<T>>(std::forward<Eigen::Tensor<T, 3>&&>(t), computeGrad);
    }
...
#endif // __CML_TENSORS_TENSOR4D_H__

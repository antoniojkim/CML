    ...
    */
    ...
    class Tensor {
        ...
        public:
            ...
            /*
                Get coefficients for scalar tensors where:
                    - "N" is the batch number
            */
            virtual T& at(const int& N) = 0;
            /*
                Get coefficients for vector tensors where:
                    - "N" is the batch number
                    - "R" is the row number
            */
            virtual T& at(const int& N, const int& R) = 0;
            /*
                Get coefficients for matrix tensors where:
                    - "N" is the batch number
                    - "R" is the row number
                    - "C" is the columns number
            */
            virtual T& at(const int& N, const int& R, const int& C) = 0;
            /*
                Get coefficients for 3d tensors where:
                    ...
                    - "W" is the width
            */
            virtual T& at(const int& N, const int& C, const int& H, const int& W) = 0;
        
            inline T& operator()(const int& N){ return this->at(N); }
            inline T& operator()(const int& N, const int& R){ return this->at(N, R); }
            inline T& operator()(const int& R, const int& C, const int& D){ return this->at(R, C, D); }
            
            
            virtual void set(std::vector<T>&& values) = 0;
            virtual void set(std::vector<std::vector<T>>&& values) = 0;
            virtual void set(std::vector<std::vector<std::vector<T>>>&& values) = 0;
            
            inline void operator=(std::vector<T>&& values){
                this->set(std::forward<std::vector<T>&&>(values));
            };
            inline void operator=(std::vector<std::vector<T>>&& values){
                this->set(std::forward<std::vector<std::vector<T>>&&>(values));
            };
            inline void operator=(std::vector<std::vector<std::vector<T>>>&& values){
                this->set(std::forward<std::vector<std::vector<std::vector<T>>>&&>(values));
            };
